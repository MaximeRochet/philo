...
/*   Updated: 2021/09/27 16:08:14 by mrochet          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
{
	//if(d_philo->last_eat + data->t_sleep + data->t_eat > d_philo->last_eat + data->t_die)
...
int before_eat(t_philo *d_philo, t_data *data)
{
	...
	{
		if(d_philo->last_eat + data->t_eat*2 > d_philo->last_eat + data->t_die)
			return(print_death(d_philo, data));
	}
	else if(data->n_philo % 2 == 0 && data->t_eat < data->t_sleep)
	{
		if(d_philo->last_eat + data->t_eat + data->t_sleep > d_philo->last_eat + data->t_die)
			return(print_death(d_philo, data));
	}
	...
	{
		if(d_philo->last_eat + data->t_eat*3 > d_philo->last_eat + data->t_die)
			return(print_death(d_philo, data));
	}
	return(1);
}
